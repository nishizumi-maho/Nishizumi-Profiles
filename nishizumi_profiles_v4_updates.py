...
GITHUB_RELEASES_LATEST_API = f"https://api.github.com/repos/{GITHUB_RELEASE_OWNER}/{GITHUB_RELEASE_REPO}/releases/latest"
GITHUB_RELEASES_PAGE = f"https://github.com/{GITHUB_RELEASE_OWNER}/{GITHUB_RELEASE_REPO}/releases"
...
def parse_version_tag(tag: str | None) -> tuple[int, ...]:
...
def compare_version_tags(current_tag: str, latest_tag: str) -> int:
    current = list(parse_version_tag(current_tag))
    latest = list(parse_version_tag(latest_tag))
    size = max(len(current), len(latest))
    current.extend([0] * (size - len(current)))
    latest.extend([0] * (size - len(latest)))
    if tuple(current) < tuple(latest):
        return -1
    if tuple(current) > tuple(latest):
        return 1
    return 0
...
def fetch_latest_github_release(timeout: float = 6.0) -> dict[str, Any]:
    request = urllib.request.Request(
        GITHUB_RELEASES_LATEST_API,
        headers={
            "Accept": "application/vnd.github+json",
            "User-Agent": f"{APP_NAME}/{APP_RELEASE_TAG}",
        ...
        while not self.stop_event.is_set():
            ...
            if sim_running and connected:
                ...
                if combo is not None:
                    combo_key = combo.combo_key(grouping_mode)
                    selected_entry = self.manager.register_combo(combo, renderer_file, grouping_mode)
                    ...
                    if resolved:
                        ...
                        resolved_combo_key = str(resolved_entry.get("combo_key") or combo.combo_key(resolved_grouping))
                    ...
                    if self.session is None or self.session.get("combo_key") != combo_key or self.session.get("renderer_file") != renderer_file or self.session.get("grouping_mode") != grouping_mode:
                        ...
                        if profile_exists and enabled:
                            ...
                            if renderer_matches and app_matches:
                                ...
                                self.log(
                                    f"Known profile ({resolved_grouping}) is different from the active file ({mismatch_text}). "
                    ...
                    if bool(self.session.get("profile_apply_pending")):
                        ...
                        if should_save:
                            ...
                            if save_app_ini_global and save_app_ini_entry:
                                ...
                                self.log("Per-car app.ini autosave skipped by settings")
                        else:
                            if is_new_combo and autosave:
                                self.log("Autosave skipped for this new combo by user choice")
                            else:
...
class App(QMainWindow):
    def __init__(self) -> None:
        ...
        self.update_result_lock = threading.Lock()
        self.pending_update_result: dict[str, Any] | None = None
        ...
        self.monitor = MonitorService(
            ...
            confirm_new_combo_save=self.confirm_new_combo_save,
        )
        self.monitor.start()
        ...
        if self.manager.get_bool_setting("check_updates_automatically", True):
    ...
    def _drain_log_queue(self) -> None:
        ...
        self._process_pending_update_result()
    ...
    def _process_pending_prompt(self) -> None:
        ...
        answer = QMessageBox.question(
            ...
            "A new combo was detected and closed manually.\n\n"
            f"{combo.label()}\n\n"
            "Save this as a new profile now?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.Yes,
        )
    ...
    def check_for_updates(self, manual: bool = False) -> None:
        ...
        self.set_status("Checking GitHub releases for updates...")

        def worker() -> None:
            result: dict[str, Any]
            try:
                data = fetch_latest_github_release()
                latest_tag = str(data.get("tag_name") or "").strip()
                release_url = str(data.get("html_url") or GITHUB_RELEASES_PAGE).strip() or GITHUB_RELEASES_PAGE
                if not latest_tag:
                    raise RuntimeError("No published release tag was returned by GitHub")
                cmp_result = compare_version_tags(APP_RELEASE_TAG, latest_tag)
                result = {
                    "manual": manual,
                    "ok": True,
                ...
                result = {"manual": manual, "ok": False, "message": f"GitHub returned HTTP {exc.code}"}
            except urllib.error.URLError as exc:
                result = {"manual": manual, "ok": False, "message": f"Network error while checking updates: {exc.reason}"}
            except Exception as exc:
                result = {"manual": manual, "ok": False, "message": f"Update check failed: {exc}"}

            with self.update_result_lock:
                self.pending_update_result = result

        threading.Thread(target=worker, name="GitHubReleaseCheck", daemon=True).start()

    def _process_pending_update_result(self) -> None:
        with self.update_result_lock:
        ...
        if not result.get("ok"):
            message = str(result.get("message") or "Update check failed")
            self.set_status(message)
            if manual:
                QMessageBox.warning(self, "Update check failed", message)
            return

        latest_tag = str(result.get("latest_tag") or APP_RELEASE_TAG)
        release_url = str(result.get("release_url") or GITHUB_RELEASES_PAGE)
        if bool(result.get("has_update")):
            self.set_status(f"Update available: {latest_tag}")
            answer = QMessageBox.question(
                self,
                "Update available",
                f"Current version: {APP_RELEASE_TAG}\nLatest version: {latest_tag}\n\nOpen the GitHub releases page now?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                QMessageBox.StandardButton.Yes,
            )
            if answer == QMessageBox.StandardButton.Yes:
            ...
            self.set_status(f"You are up to date ({APP_RELEASE_TAG})")
            if manual:
                QMessageBox.information(self, "No update available", f"You are already on the latest version: {APP_RELEASE_TAG}")
    ...
    def _build_ui(self) -> None:
        ...
        self.prompt_new_combo_check = QCheckBox("Prompt before saving new combos")
        self.prompt_new_combo_check.setChecked(self.manager.get_bool_setting("prompt_new_combo_save", True))
        self.prompt_new_combo_check.stateChanged.connect(
            lambda _: self.manager.set_bool_setting("prompt_new_combo_save", self.prompt_new_combo_check.isChecked())
        )
        toggle_row.addWidget(self.prompt_new_combo_check)

        self.global_app_ini_check = QCheckBox("Autosave app.ini globally")
        self.global_app_ini_check.setChecked(self.manager.get_bool_setting("save_app_ini_on_autosave_global", True))
        self.global_app_ini_check.stateChanged.connect(
            lambda _: self.manager.set_bool_setting("save_app_ini_on_autosave_global", self.global_app_ini_check.isChecked())
        )
        toggle_row.addWidget(self.global_app_ini_check)

        self.auto_update_check = QCheckBox("Check for updates automatically")
        self.auto_update_check.setChecked(self.manager.get_bool_setting("check_updates_automatically", True))
        self.auto_update_check.stateChanged.connect(
            lambda _: self.manager.set_bool_setting("check_updates_automatically", self.auto_update_check.isChecked())
        )
        toggle_row.addWidget(self.auto_update_check)

        check_updates_btn = QPushButton("Check updates")
        check_updates_btn.clicked.connect(lambda: self.check_for_updates(manual=True))
        toggle_row.addWidget(check_updates_btn)
        ...
        self.combo_app_ini_check = QCheckBox("Save app.ini for this combo")
        self.combo_app_ini_check.setChecked(True)
        opt_row.addWidget(self.enabled_check)
        opt_row.addWidget(self.autosave_check)
        opt_row.addWidget(self.combo_app_ini_check)
        opt_row.addStretch(1)
        right_layout.addLayout(opt_row)

        main_actions = QHBoxLayout()
        for label, cb in [
            ("Load from sim", self.load_current_sim_combo),
        ...
        for idx, (label, cb) in enumerate([
            ...
            ("Create global backup", self.create_global_backup),
            ("Restore global backup", self.restore_global_backup),
    ...
    def set_combo_app_ini(self, save_app_ini: bool) -> None:
        self.combo_app_ini_check.setChecked(save_app_ini)

    def clear_fields(self) -> None:
        self.selected_combo_key = None
        self.fill_form(ComboInfo(), enabled=True, autosave=True)
        self.set_combo_app_ini(True)
    ...
    def on_table_select(self) -> None:
        ...
        self.set_combo_app_ini(bool(entry.get("save_app_ini_on_autosave", True)))
    ...
    def save_profile_from_active_ini(self) -> None:
        ...
        try:
            ...
            app_saved = False
            if self.global_app_ini_check.isChecked() and self.combo_app_ini_check.isChecked():
                try:
            ...
            self.manager.update_entry_options(
                ...
                self.combo_app_ini_check.isChecked(),
